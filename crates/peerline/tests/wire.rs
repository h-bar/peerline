...
#[test]
fn response_err_rpc_preserves_full_error() {
    let r = peer::response_err_rpc(
        Some(1u64),
        RpcError {
            code: -32000,
            message: "boom".into(),
    ...
    let s = serde_json::to_string(&Frame::from(r)).unwrap();
    let back: Frame = serde_json::from_str(&s).unwrap();
    match peer::classify(back) {
        InboundKind::Response {
            id: Some(1),
            outcome: Err(e),
        } => {
            assert_eq!(e.code, -32000);
            assert_eq!(e.data, Some(json!({"k": 1})));
        }
