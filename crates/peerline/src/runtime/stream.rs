    ...
    pub(crate) fn send_error(&mut self, error: RpcError) {
        if !self.sent {
            ...
            self.outbound.finish_stream(
                self.id,
                terminal_text(p::stream_terminal_with_rpc_error(self.id, error)),
            );
        }
    }
}
...
pub struct StreamItem<R> {
    ...
    /// (0-indexed, monotonic — gaps signal drops the producer
    /// [`skip`](StreamSender::skip)ped past), or `-1` if this item
    /// arrived bundled on the terminal frame.
    pub seq: i64,
    /// The decoded payload.
    pub data: R,
}
...
impl<R> StreamReceiver<R> {
    ...
    /// End local tracking for this stream: drop the registry entry so
    /// late frames for the finished id are discarded (instead of
    /// queueing into a dead channel forever) and `active_streams`
    /// counts only live streams. With `cancel`, additionally tell the
    /// producer to stop — used when the consumer abandons a stream
    /// that may still be producing, not when its terminal was seen.
    fn finish(&mut self, cancel: bool) {
        if let Some(id) = self.id.take() {
            if cancel
                && let Ok(notif) = p::notification(super::STREAM_CANCEL_OP, &CancelArgs { id })
            {
                ...
                Poll::Ready(Some(frame)) => {
                    ...
                    if let Some(data) = frame.data {
                        let item = match data.deserialize() {
                            ...
                            Err(e) => {
                                // Local decode failure: the consumer is
                                // abandoning a stream the producer may
                                // still be feeding — cancel upstream.
                                this.finish(true);
...
impl<R> Drop for StreamReceiver<R> {
    fn drop(&mut self) {
        ...
        // reserved cancel notification is intercepted on the far side
